...
def build_ce_dnn(K, SNR, savefile, learning_rate=1e-3, training_epochs=2000, batch_size=50, nh1=500, nh2=250, test_flag=False, cp_flag=True):
    ...
    dense1 = Dense(nh1, activation=tf.nn.relu, name='dense1')(nn_input)
    dense2 = Dense(nh2, activation=tf.nn.relu, name='dense2')(dense1)
    output_layer = Dense(n_output, activation=None, name='output_layer')(dense2)

    tmp = tf.reshape(output_layer, [-1, 2, K])
    tmp = tf.transpose(tmp, [0, 2, 1])
    H_out = tf.reshape(tmp, [-1, n_output])
