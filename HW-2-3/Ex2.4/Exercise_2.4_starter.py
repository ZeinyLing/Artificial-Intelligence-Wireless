...
def generator_conditional(z, conditioning):
    ...
    # 將 ReLU 替換為 Leaky ReLU (alpha 預設通常設為 0.2)
    G_h1 = tf.nn.leaky_relu(tf.matmul(z_combine, G_W1) + G_b1, alpha=0.2)
    G_h2 = tf.nn.leaky_relu(tf.matmul(G_h1, G_W2) + G_b2, alpha=0.2)
    G_h3 = tf.nn.leaky_relu(tf.matmul(G_h2, G_W3) + G_b3, alpha=0.2)

    # 輸出層保持 Linear，因為我們的目標訊號在實數/複數平面上，不需要被 Sigmoid 或 Tanh 截斷
...
def discriminator_conditional(X, conditioning):
    ...
    Returns:
        D_prob: Probability that the input is real (Optional for WGAN).
        D_logit: Raw output logits of the discriminator (Used for WGAN-GP loss).
    """
    z_combine = tf.concat([X, conditioning], 1)

    # 將 ReLU 替換為 Leaky ReLU，確保梯度在負值區間也能順暢回傳給 Generator
    D_h1_real = tf.nn.leaky_relu(tf.matmul(z_combine / 4.0, D_W1) + D_b1, alpha=0.2)
    D_h2_real = tf.nn.leaky_relu(tf.matmul(D_h1_real, D_W2) + D_b2, alpha=0.2)
    D_h3_real = tf.nn.leaky_relu(tf.matmul(D_h2_real, D_W3) + D_b3, alpha=0.2)

    D_logit = tf.matmul(D_h3_real, D_W4) + D_b4

    # 傳統 GAN 使用 Sigmoid，但您的 Loss 設定是 WGAN-GP，
    # WGAN-GP 實際上主要是看 D_logit，D_prob 這裡只是保留原程式結構。
...
def generate_real_samples_with_labels_Rayleigh(h_dataset, number=100):
    ...
    # h_dataset is assumed to be an array of complex numbers.
    h_samples = np.random.choice(h_dataset, size=number)

    # 2. Generate random QAM symbols (16-QAM based on your global mean_set_QAM).
    ...
    x_symbols = np.random.choice(qam_constellation, size=number)

    # 3. Create the received signal (y = hx + n).
    # Calculate noiseless received signal
    y_clean = h_samples * x_symbols

    # Generate AWGN (matching the 0.03 variance used in your plotting section)
    ...
    # Format received data into shape [number, 2] for Real and Imaginary parts
    received_data = np.column_stack((np.real(y_received), np.imag(y_received)))

    # 4. Construct the conditioning vector.
    # Format: [Re(x), Im(x), Re(h), Im(h)] scaled by 1/3 to match generator inputs
    cond_real_x = np.real(x_symbols)
    cond_imag_x = np.imag(x_symbols)
    cond_real_h = np.real(h_samples)
    cond_imag_h = np.imag(h_samples)

    conditioning = np.column_stack((cond_real_x, cond_imag_x, cond_real_h, cond_imag_h)) / 3.0
...
# ────────────────────────  Graph Construction  ───────────────────────────── #
D_W1 = tf.Variable(xavier_init([2 + condition_dim, 128]))
...
for it in range(750000):
    ...
    if (it + 1) % plot_every == 0:
        ...
        for channel_idx in range(10):
            ...
            plt.ylabel(ylabel)
            plt.show()
            #plt.savefig(save_fig_path + '/' + str(channel_idx) + '_{}_noise_1.eps'.format(str(i).zfill(3)),bbox_inches='tight')
            #plt.savefig(save_fig_path + '/' + str(channel_idx) + '_{}_noise_1.png'.format(str(i).zfill(3)),bbox_inches='tight')
