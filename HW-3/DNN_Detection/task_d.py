# -*- coding: utf-8 -*-
"""
Task (d) PyTorch Version Using Original utils.py

Task (d):
- Revert to QPSK: mu = 2
- Use a single large DNN
- Set pred_range = np.arange(0, 128)
- Set n_output = 128
...
def Modulation(bits, mu):
    ...
    if mu == 2:
        ...
        raise ValueError("Task (d) uses QPSK only, so mu must be 2.")
...
MU = 2                      # QPSK
N_BITS = K * MU             # 64 * 2 = 128 bits

# Task (d): single large DNN predicts all 128 bits
PRED_RANGE = np.arange(0, 128)
N_OUTPUT = len(PRED_RANGE)  # 128

# You can keep the same SNR / pilot setting for comparison
...
# 20000 epochs is usually too large.
# Start with 500. If BER is unstable, increase to 1000.
EPOCHS = 20000
BATCH_SIZE = 512
LR = 1e-3
...
SAVE_DIR = "./task_d_single_large_dnn_results"
...
def generate_dataset(num_samples, snr_db, P):
    ...
    y:
        all 128 target bits for single large DNN
...
def main():
    ...
    for P in PILOT_LIST:
        ...
        for snr_db in SNR_LIST:
            print("\n" + "=" * 80)
            print(f"Task (d) Single Large DNN | QPSK | SNR = {snr_db} dB | Pilots = {P}")
            print("=" * 80)
            ...
            print("Training single large DNN...")
            ...
            print(
                f"Result | Task (d) | QPSK | SNR = {snr_db} dB | "
                f"Pilots = {P} | BER = {ber:.8f}"
            )

    # Save txt results
    txt_path = os.path.join(SAVE_DIR, "task_d_single_large_dnn_ber_results.txt")

    with open(txt_path, "w") as f:
        f.write("Task (d) Single Large DNN BER Results\n")
        f.write(f"MU = {MU}\n")
    ...
    # Save CSV results
    csv_path = os.path.join(SAVE_DIR, "task_d_single_large_dnn_ber_results.csv")
    ...
    plt.xlabel("SNR (dB)")
    plt.ylabel("BER")
    plt.title("Task (d): Single Large DNN BER vs. SNR")
    plt.grid(True, which="both")
    plt.legend()
    plt.tight_layout()

    fig_path = os.path.join(SAVE_DIR, "task_d_single_large_dnn_BER_vs_SNR.png")
